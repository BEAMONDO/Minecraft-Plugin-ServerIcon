package davigamer161.servericon.comandos;
...
public class ComandoPrincipal implements CommandExecutor{
    ...
    public ComandoPrincipal(ServerIcon plugin){
        this.plugin = plugin;
    }
...
//----------------------------------------Desde aqui---------------------------------------//
                if(args[0].equalsIgnoreCase("version")){
                    if(sender instanceof Player && (jugador.hasPermission("servericon.version"))){
...
//----------------------------------------Desde aqui---------------------------------------//
                    ...
                    if(sender instanceof Player && (jugador.hasPermission("servericon.help"))){
 ...
 //----------------------------------------Desde aqui---------------------------------------//
                    ...
                    if(sender instanceof Player && (jugador.hasPermission("servericon.reload"))){
                    plugin.reloadConfig();
                    plugin.reloadMessages();
...
//----------------------------------------Hasta aqui---------------------------------------//
                ...
                if(sender instanceof Player && (jugador.hasPermission("servericon.help"))){
                    ...
                    jugador.sendMessage(ChatColor.translateAlternateColorCodes('&', mensaje.replaceAll("%player%", jugador.getName()).replaceAll("%plugin%", plugin.nombre).replaceAll("%version%", plugin.version)));
                return true;
                }else if(sender instanceof Player && !(jugador.hasPermission("servericon.help"))){
                    if(config.getString(path).equals("true")){
                        String mensaje = messages.getString("Messages.no-perm");
                        jugador.sendMessage(ChatColor.translateAlternateColorCodes('&', mensaje.replaceAll("%player%", jugador.getName()).replaceAll("%plugin%", plugin.nombre).replaceAll("%version%", plugin.version)));
                    }
